...
public class ProyectoService implements IProyectoService {
    ...
    public ProyectoResponseDTO crearProyecto(ProyectoRequestDTO dto) {
        ...
        Columna backlog = new Columna();
        backlog.setNombreColumna("Product Backlog");
        backlog.setProyecto(proyecto);
        backlog.setTablero(null);
        backlog.setPosicion(0);
        columnaRepository.save(backlog);
    ...
    public ProyectoResponseDTO obtenerProyectoPorId(Long id) {
        ...
        boolean miembro = proyectoUsuarioRepository
                .findByProyecto_IdProyectoAndUsuario_IdUsuario(id, usuario.getIdUsuario()).isPresent();
    ...
    public void eliminarProyecto(Long id) {
        ...
        if (!esCreador && !esAdmin) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                    "Solo el creador o un admin del proyecto puede eliminarlo");
        }
    ...
    public List<ProyectoResponseDTO> obtenerProyectosPorUsuario() {
        Usuario usuario = authService.getUsuarioAutenticado();
        List<ProyectoUsuario> proyectosUsuario = proyectoUsuarioRepository
