...
import { getMisNotificacionesRequest, marcarComoLeidaRequest, marcarTodasComoLeidasRequest } from "../services/notificationService";
...
const NotificationBell = () => {
    ...
    const handleClickNotificacion = async (notif) => {
        if (!notif.leida) {
            try {
                const updated = await marcarComoLeidaRequest(notif.idNotificacion);
                setNotificaciones(prev =>
                    prev.map(n => n.idNotificacion === notif.idNotificacion ? updated : n)
                );
            } catch (err) {
                console.error(err);
            }
    ...
    return (
        <div className="relative" ref={bellRef}>
            ...
            {open && (
                <div
                    ref={panelRef}
                    className="absolute right-0 top-11 w-80 bg-white dark:bg-[#1a1a1a] border border-slate-200
                     dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50"
                >
                    {/* Header del panel */}
                    ...
                    <div className="max-h-96 overflow-y-auto">
                        ...
                        {!loading && notificaciones.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-10 gap-2">
                                ...
                                <p className="text-slate-400 dark:text-white/30 text-xs">Sin notificaciones</p>
                            </div>
                        )}

                        {!loading && notificaciones.map(notif => {
                            const tipoStyle = TIPO_ICON[notif.tipo] ?? TIPO_ICON.ASIGNACION;
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-start gap-3 px-4 py-3 text-left transition-colors
                                        ...
                                        !notif.leida ? "bg-indigo-50/50 dark:bg-white/3" : ""
                                    }`}
                                >
                                    {/* Ícono tipo */}
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                                        ...
                                        <span className={`material-symbols-rounded text-[16px] ${
                                            !notif.leida ? tipoStyle.color : "text-slate-400 dark:text-white/30"
                                        }`}>
                                            {tipoStyle.icon}
                                        </span>
                                    </div>
